/**
 * ONE-OFF DEMO SEED — TWO area managers, one per city (Riyadh / Jeddah),
 * zero transactions. Same conventions as demo-org-only.js: every employee
 * has an Iqama ID, every POS has a POS ID + POS Config, area-bulk upload
 * enabled. One shared Deputy and one shared Collector serve both areas —
 * demonstrating that a single Deputy/Collector can cover multiple area
 * managers' clusters.
 *
 * Run: node tests/demo-two-areas.js [port]
 */
...
var PORT = Number(process.argv[2] || 8995);
...
console.log('Seeding TWO-area-manager (Riyadh / Jeddah) demo scenario, zero transactions...');
...
var AREAS = [
  {
    areaName: 'Riyadh Area', areaMgrName: 'Faisal Al-Otaibi (Area Manager)', areaMgrEmail: 'faisal@bestgas.sa',
    city: 'Riyadh',
    locs: [
  ...
  {
    areaName: 'Jeddah Area', areaMgrName: 'Saad Al-Ghamdi (Area Manager)', areaMgrEmail: 'saad.ghamdi@bestgas.sa',
    city: 'Jeddah',
    locs: [
      ['Rawdah',  'Fahad (Store Manager)',  'Yazan (Driver)',   '2011122239', '2011122240', 'POS-RWD-01'],
      ['Salamah', 'Sultan (Store Manager)', 'Ibrahim (Driver)', '2011122241', '2011122242', 'POS-SLM-01'],
      ['Hamra',   'Waleed (Store Manager)', 'Hamzah (Driver)',  '2011122243', '2011122244', 'POS-HMR-01']
    ]
  }
];
...
AREAS.forEach(function (area, ai) {
  ...
  area.locs.forEach(function (row, i) {
    ...
    var loc = call({ action: 'adminSaveEntity', token: adminTok, kind: 'location', data: { city: area.city, name: name, clusterId: cluster.id } }).entity;
    ...
    var posEntity = call({
      ...
      data: { ownerType: 'car', ownerId: car.id, label: name + ' POS Machine', assignedUserId: drv.id, posId: posId, posConfig: 'merchantId=9913' + (1000 + ai * 10 + i) + ';terminal=T' + idx }
    }).entity;
    allLocRows.push({ area: area.areaName, city: area.city, name: name, mgrEmail: 'mgr' + idx + '@bestgas.sa', drvEmail: 'drv' + idx + '@bestgas.sa' });
  });
...
allLocRows.forEach(function (r) {
  console.log(r.area + ' / ' + r.city + ' / ' + r.name + ':  ' + r.mgrEmail + ' temp=' + lastInviteFor(r.mgrEmail) + '   ' + r.drvEmail + ' temp=' + lastInviteFor(r.drvEmail));
});
console.log('');
